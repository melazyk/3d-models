...
| File | What it is | Use |
|---|---|---|
| `snap_bare_full.stl` | openGrid snap body, 6.8 mm (full board), no thread | fuse to an accessory's flat back → it clicks straight onto the wall |
...
- **Permanent** — `mounts.snaps("basic_full", cols, rows)` unions bare/basic snaps
  onto the z=0 face. Simplest; the whole accessory clicks onto openGrid.
...
The **CC BY-NC** on the Multiconnect backer means anything using
`mounts.slots(..., "multiconnect", ...)` inherits a non-commercial restriction.
The bare/basic openGrid snaps are CC BY (commercial OK).
